...
class DateTimeConverterShould {
    ...
    class TodayTests {
        ...
        void convertTodayStringCorrectly() {
            LocalDate today = LocalDate.of(2020, 9, 1);
            LocalDateTime result = DateTimeConverter.convertStringToDateTime("today 1:00 pm",
                    today);
            assertEquals(result, LocalDateTime.of(2020, 9, 1, 13, 0),
    ...
    void convertCorrectPatternToDateTime() {
        LocalDateTime result = DateTimeConverter.convertStringToDateTime("9/2/2020 1:00 pm",
                LocalDate.of(2020, 9, 2));
        assertEquals(result, LocalDateTime.of(2020, 9, 2, 13, 0));
    }
    ...
    void throwExceptionIfIncorrectPatternProvided() {
        Throwable error = assertThrows(RuntimeException.class, () ->
                DateTimeConverter.convertStringToDateTime("9/2/2020 100 pm",
                        LocalDate.of(2020, 9, 2)));
        assertEquals("Unable to create date time from: [9/2/2020 100 pm], " +
                "please enter with format [M/d/yyyy h:mm a], " +
                "Text '9/2/2020 100 PM' could not be parsed at index 12", error.getMessage());
    }
}